    ...
    implements SurfaceHolder.Callback
{
    ...
    private CannonThread cannonThread;  //Controls game loop
    private Activity activity;  //for Game Over dialog
    private boolean dialogIsDisplayed = false;

    //Constants for game play
    public static final int TARGET_PIECES = 7;  //Number of Targets
    public static final int MISS_PENALTY = 2;   //Penalty on miss
    public static final int HIT_REWARD = 3;     //Seconds added on hit
    ...
    private Line target;               // start and end points of the target, defined in class Line
    ...
    public CannonView(Context context, AttributeSet attrs) {
        super(context, attrs);  //Superclass constructor
        ...
        blocker = new Line();  //create blocker as a Line
        target = new Line();  //create target as a Line
        cannonball = new Point(); //create cannonball as a Point
        ...
        //Construct Paints, configured in method onSizeChanged
        textPaint = new Paint();
    ...
    /*
      In this class we are overriding class View's various methods,
      just like we override class Activity's methods in class MainActivity.
      The following overrides onSizeChanged method to scale the objects according
      to the device's pixel width and height.
    */
    @Override
    protected void onSizeChanged (int w, int h, int oldw, int oldh){
        ...
        screenWidth = w;
        screenHeight = h;
        cannonBaseRadius = h / 18;  //cannon base radius is 1/18th the screen height
        cannonLength = w / 8; //cannon length is 1/8th the screen width

        cannonballRadius = w / 36;
        cannonballSpeed = w * 3 / 2;  //cannonball speed multiplier

        lineWidth = w / 24;  //target and blocker are 1/24th screen width

        //configure blocker instance variables
        blockerDistance = w * 5/8;  // blocker 5/8 screen width from left
        blockerBeginning = h / 8;   //distance from top relative to height
        blockerEnd = h * 3/8;       //distance from top relative to height
